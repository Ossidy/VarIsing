...
def train(niters=20000, batch_size=16, lr_D=0.00005, lr_G=0.00005, Diter=5, weight_penalty=10,
	...
	data_iter = dataloader.dataloader(batchsize=batch_size, fileformat='.dat', is_json=False, dummy_dim=True) # function should be defined as iterator for images
	...
	for epoch in range(niters):
		...
		for i in range(Diters): # need change !!!!!!!!!!!!!!!!!!!
			...
			errD_real_vec = netD(real_data)
			errD_real = errD_real_vec.mean(0)
			# errD_real.backward(one)

			gen_input.data.uniform_(-1,1)
			# fake = Variable(netG(noisev).data)
			fake_data = netG(gen_input)

			errD_fake_vec = netD(fake_data)
			# print(errD_fake_vec)
		...
		if gen_iterations % 50 == 0:
			...
			losses.append([errD.cpu().data.numpy()[0], TEMP[0].cpu().data[0], TEMP[1].cpu().data[0], errG.cpu().data[0], TEMP[2].cpu().data[0]])
			print("iter: {}, errD: {}, errD_real: {}, errD_fake: {}, errG: {}".format(gen_iterations, errD.cpu().data[0], TEMP[0].cpu().data[0], TEMP[1].cpu().data[0], errG.cpu().data[0]))
			print(get_observables(np.array(fake_data.cpu().data.tolist()[0][0])))
...
if __name__ == "__main__":
	...
	train(niters=flags.niters, batch_size=flags.batch_size, lr_D=flags.D_learning_rate, lr_G=flags.G_learning_rate, 
		  ...
		  save_model_path="./saved_models/", save_image_path="./saved_images/", load_model_path=None)
